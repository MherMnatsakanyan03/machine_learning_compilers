...
MiniJit::Kernel::~Kernel() noexcept
{
	release_memory();
}

void MiniJit::Kernel::add_instr(uint32_t ins)
{
	m_buffer.push_back(ins);
}

std::size_t MiniJit::Kernel::get_size() const
{
	return m_buffer.size() * 4;
}

void *MiniJit::Kernel::alloc_mmap(std::size_t num_bytes) const
{
	void *l_mem = mmap(0,
					   num_bytes,
...
void MiniJit::Kernel::release_mmap(std::size_t num_bytes,
								   void *mem) const
{
...
void MiniJit::Kernel::set_exec(std::size_t num_bytes,
							   void *mem) const
{
...
void MiniJit::Kernel::set_kernel()
{
	release_memory();
...
void const *MiniJit::Kernel::get_kernel() const
{
	return m_kernel;
}

void MiniJit::Kernel::release_memory()
{
	if (m_kernel != nullptr)
	{
...
void MiniJit::Kernel::write(char const *path) const
{
